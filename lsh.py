...
def distance(im1, im2):
    return sum(sum(abs(im1 - im2)))

def bruteForceSearch(data, index):
    im1 = data[index]
    minIndex = 0
    minDist = distance(im1, data[minIndex])
    for r in range(1, 1500):
        distanceAux = distance(im1, data[r])
        if distanceAux < minDist:
            minDist = distanceAux
            minIndex = r
    return minIndex

def findNearestNeighbor(me, index, cands):
    if len(cands) == 0:
        return None
    minIndex = -1
    im1 = me.data[index]
    minDist = -1
    for r in cands:
        distanceAux = distance(im1, me.data[r])
        if distanceAux < minDist or minDist == -1:
            minDist = distanceAux
            minIndex = r
    return minIndex
...
def lsh_search(args):
    ...
    for r in range(1500, 1505):
        im = me.data[r]
        cands = me.candidates(im)
        nearestNeighbor = findNearestNeighbor(me, r, cands)
        if nearestNeighbor is None:
            distanceim = - 1
        else:
            distanceim = distance(im, me.data[nearestNeighbor])
        print("the nearest neighbor for ", r , " is ", nearestNeighbor , " with distance ", distanceim)
...
def bruteForce():
    ...
    for r in range(1500, 1505):
        #im = data[r]
        nearestNeighbor = bruteForceSearch(data, r)
        distanceim = distance(data[r], data[nearestNeighbor])
        print("the nearest neighbor for ", r , " is ", nearestNeighbor , " with distance ", distanceim)
        #print("there are %4d candidates for image %4d" % (len(cands), r))
    return
